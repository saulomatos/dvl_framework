...
class SumoPymooProblem(Problem):
    ...
    def __init__(self):
        exit("class SumoPymooProblem(Problem):")
...
class DVLFramework():
    ...
    def __init__(self,
        ...
        self.hv_ref = self.problem.referenceHV #.referenceHV()
        ...
        #print (f"self.problem.num_variables = {self.problem.num_variables}")
        #print (f"self.problem.num_objectives = {self.problem.num_objectives}")
        #print("fim INIT da class DVLFramework()")
    ...
    def execute(self):
        ...
        if diff > 0:
            ...
            algorithm = MOEAD(
            ref_dirs=reference_points,
            sampling=np.asarray(self.best_solutions),
            n_neighbors=15,
            decomposition=PBI(),
    ...
    def executeRealProblem(self):
        #print("\n\nInício dentro após instanciar dvl e dentro do def runDVLFramework()\n")
        '''
        print(f"self.lowerbound = {self.problem.lowerbound}")
        print(f"self.lowerbound = {self.problem.upperbound}")
        print (f"self.problem.num_variables = {self.problem.num_variables}")
        print (f"self.problem.num_objectives = {self.problem.num_objectives}")
        exit("a")
        '''
        ...
        #solutions = (pyDOE.lhs(self.problem.num_variables, samples=self.samples) * 100) + 20

        solutions = (pyDOE.lhs(self.problem.num_variables, samples=self.samples) 
                     * (self.problem.upperbound - self.problem.lowerbound)) + self.problem.lowerbound
        ...
        if self.problem.num_objectives == 10 or self.problem.num_objectives == 6:
            ...
            exit("def executeRealProblem() and number of objetiver not in [6, 10]")
            reference_points = np.asarray(generate_reference_points(self.problem.num_objectives, self.problem.num_variables))

        print(f"executeRealProblem   num_objectives = {self.problem.num_objectives}\nreference_points = {reference_points}")
        ...
        print("antes unique:", len(new_solutions))

        new_solutions = np.unique(new_array, axis=0)

        print("depois unique:", len(new_solutions))
        ...
        print("hv_ref =", self.hv_ref)
        print("normal =", self.normal)

        hv = hvwfg.wfg(new_objectives, self.hv_ref)

        print("hv bruto =", hv)

        best_hv = hv / self.normal

        print("hv normalizado =", best_hv)
        ...
        if diff_time > 10:
            ...
            print("ref_dirs:", len(reference_points))
            print("best_solutions:", len(self.best_solutions))
            print("reference_points.shape =", reference_points.shape)
            print("best_solutions.shape =", np.asarray(self.best_solutions).shape)
            print("new_solutions.shape =", np.asarray(new_solutions).shape)

            #exit("e ae")
            algorithm = MOEAD(
                #pop_size=reference_points.shape[0],
                ref_dirs=reference_points,
                #sampling=np.asarray(self.best_solutions),
        ...
        with open('./Results/Experimento3/Bruto/Scenario_'+str(self.problem.scenario)+'_Flow_'+self.problem.flow+'_time_'+str(self.num_process_time)+'_DVL2_'+str(self.num_order)+'.txt', 'w') as f:
        ...
        var2.to_excel('./Results/Experimento3/Bruto/Scenario_'+str(self.problem.scenario)+'_Flow_'+self.problem.flow+'_result_'+str(self.num_process_time)+'_DVL2_'+str(self.num_order)+'.xlsx',header=False, index=False)
